...
class WebpconverterAPIClientError(Exception):
...
class ValidationError(WebpconverterAPIClientError):
...
class WebpconverterAPIClient:
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the WebpconverterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises WebpconverterAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/webpconverter'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[WebpconverterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns WebPConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises WebpconverterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise WebpconverterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise WebpconverterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise WebpconverterAPIClientError(f"Request failed: {str(e)}")
    ...
    def execute_with_file(self, files: Dict[str, Any], params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self._log(f"Request exception: {str(e)}")
            raise WebpconverterAPIClientError(f"File upload failed: {str(e)}")
    ...
    def execute_with_url(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not url or not isinstance(url, str):
            raise WebpconverterAPIClientError("URL must be provided as a string.")

        if not re.match(r'^https?://.+', url, re.IGNORECASE):
            raise WebpconverterAPIClientError("Invalid URL format. URL must start with http:// or https://")
